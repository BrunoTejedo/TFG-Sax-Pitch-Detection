...
audiofile_names = utilsPEP.list_audiofile_names(level)
audiofiles = utilsPEP.list_audiofiles(audiofile_names, level)

def process_audiofiles(loader_class, pitch_computation_func):
    path = utilsPEP.mkdir_results(level, str_algorithm, str_loader, sample_rate, frame_size, hop_size)
    if algorithm != 3:
        for j, i in enumerate(audiofiles):
            ...
            utilsPEP.save_midi_csv(path, audiofile_names[j], notes, onsets, durations)
            utilsPEP.save_all_csv(path, audiofile_names[j], pitch_values, pitch_confidence, pitch_times)
            utilsPEP.plot_audio(audio, sample_rate, path, audiofile_names[j])
            utilsPEP.plot_estimated_pitch(pitch_times, pitch_values, pitch_confidence, path, audiofile_names[j])
    else:
        for j, i in enumerate(audiofiles):
            loader = loader_class(filename=i, sampleRate=sample_rate)
            audio = loader()
...
if algorithm == 1:
    ...
    pitch_computation_func = lambda audio: utilsPEP.yin_computation(audio, frame_size, hop_size, pitch_extractor)
    ...
    pitch_computation_func = lambda audio: utilsPEP.yin_fft_computation(audio, frame_size, hop_size, pitch_extractor, windowing, spectrum)
    ...
    sample_rate = 16000
    frame_size = 0
    graph = utilsPEP.ask_about_crepe()
    str_algorithm, pitch_extractor = utilsPEP.decide_graph_crepe(graph, hop_size)
